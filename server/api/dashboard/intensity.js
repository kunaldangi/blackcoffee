...
export async function intensity(req, res){
    try {
        console.log(req.body);
        let data = {};

        let options = { projection: { _id: 0, intensity: 1 }, };
        ...
        for (let i = 0; i < result.length; i++) {
            if(typeof(result[i].intensity) == "number") {
                if(data[result[i][req.body.filter]] == undefined) {
                    data[result[i][req.body.filter]] = 0;
                }
                data[result[i][req.body.filter]] += parseInt(result[i].intensity);
            }
        }
        ...
        console.log(`API ERROR (/api/dashboard/intensity): ${error}`);
