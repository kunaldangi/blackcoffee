...
export async function impact(req, res){
    try {
        ...
        let options = { projection: { _id: 0, impact: 1 }, };
        ...
        for (let i = 0; i < result.length; i++) {
            if(typeof(result[i].impact) == "number") {
                if(data[result[i][req.body.filter]] == undefined) {
                    data[result[i][req.body.filter]] = 0;
                }
                data[result[i][req.body.filter]] += parseInt(result[i].impact);
            }
        }
        ...
        console.log(`API ERROR (/api/dashboard/impact): ${error}`);
